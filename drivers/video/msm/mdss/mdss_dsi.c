				  ...
				  int event, void *arg)
{
	...
	ctrl_pdata = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	sync_pt_data = container_of(pdata, struct msm_sync_pt_data,
				panel_data);
	if (sync_pt_data == NULL) {
		pr_err("%s: Invalid sync data\n", __func__);
		return -EINVAL;
	}
	...
	case MDSS_EVENT_PANEL_ON:
		...
		sync_pt_data->async_wait_fences = true;
		break;
	...
	case MDSS_EVENT_PANEL_OFF:
		...
		sync_pt_data->async_wait_fences = false;
		break;
